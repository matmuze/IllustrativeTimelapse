...
//		float d = tex2D(_DepthTexture, i.uv1.xy).r;
//		d = Linear01Depth (d);
...
//		color.a = clamp( max(0.0, color.a - _CurveParams.y), 0.0, _CurveParams.x) * sign(d - _CurveParams.w);
	...
	uniform float _InnerSphere;
	uniform float _OuterSphere;
	uniform int _SplitScreen;

	float4 fragCaptureCoc (v2f i) : SV_Target 
	{	
		float4 color = float4(0,0,0,0); //tex2D (_MainTex, i.uv1.xy);
		
		float depth = tex2D(_DepthTexture, i.uv1.xy).r;		
		
		float4 projPos = float4(i.uv * 2 - 1, depth, 1);	
		projPos.y *= -1;
			
		float4 pos = mul(_ViewProjectionInverseMatrix, projPos);		
		pos /= pos.w;
		
		float dist = distance(_Target, pos);
		
//		_InnerSphere = 5;
//		_OuterSphere = 40;
		
		float f = (dist - _InnerSphere) / (_OuterSphere - _InnerSphere);
		
		color.a =  (dist < _InnerSphere) ? 0: (dist < _OuterSphere) ? f : 1; 		
		return color;
	} 
...
//		color.a = _CurveParams.z * (_CurveParams.w-d) / (d + 1e-5f);
		color.a = d; //clamp(max(0.0, color.a - _CurveParams.y), 0.0, _CurveParams.x);
